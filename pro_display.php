...
if (isset($_GET['pg']))
    ...
    $pg = 1;
if (isset($_GET['search']) || isset($_GET['skill']))
    $pg = 1;
// echo $pg;
// while( $i < $tot_pages)
// {
    ...
    <div class="mpage">
            ...
            <div class="filter-search" style="width:90% ; margin:auto; display:flex; justify-content:space-between; width:80%;">
                ...
                <div class="col">
                    ...
                    <input type="checkbox" id="php" name="skills[]" class ="skill-checkbox" value="php" <?php if (!empty($skill) && in_array("php", $skill)) echo "checked"; ?>>
                    <label for="php">PHP</label>&nbsp;
                    <input type="checkbox" id="mysql" name="skills[]"class ="skill-checkbox"  value="mysql" <?php if (!empty($skill) && in_array("mysql", $skill)) echo "checked"; ?>>
                    <label for="mysql">MySql</label>&nbsp;
                    <input type="checkbox" id="javascript" name="skills[]" class ="skill-checkbox" value="javascript" <?php if (!empty($skill) && in_array("javascript", $skill)) echo "checked"; ?>>
                    <label for="javascript">JavaScript</label>&nbsp;
                    <input type="checkbox" id="html" name="skills[]" class ="skill-checkbox" value="html" <?php if (!empty($skill) && in_array("html", $skill)) echo "checked"; ?>>
                    <label for="html">HTML</label>&nbsp;
                    <input type="checkbox" id="css" name="skills[]" class ="skill-checkbox" value="css" <?php if (!empty($skill) && in_array("css", $skill)) echo "checked"; ?>>
                    <label for="css">CSS</label>&nbsp;
                    <input type="checkbox" id="java" name="skills[]" class ="skill-checkbox" value="java" <?php if (!empty($skill) && in_array("java", $skill)) echo "checked"; ?>>
                    <label for="java">Java</label>&nbsp;
                    &nbsp;
                    <!-- <button type="submit" class="btn btn-primary"  value = "pro_display.php?">submit</button> -->
                    <a href="pro_display.php?val=<?php echo $val;?>" id = "search-skill" class="btn btn-primary">submit</a>
                    <a href="<?php if ($val == 1) echo 'pro_display.php?val=1';
                                elseif ($val == 2) echo 'pro_display.php?val=2';
                                elseif ($val == 3) echo 'pro_display.php?val=3'; ?>" class="btn btn-default ajax"> clear</a>
                </div>
            </div>
...
<script>
    ...
    jQuery(document).on("click", "a#search-skill", function (evt) {
		if (evt.which == 1) {
			if (!evt.ctrlKey && Modernizr.history) {
                var _href = jQuery(this).attr("href");
                ...
                if(search.length > 0)
                    _href += '&search=' + encodeURIComponent(search);
                if(skills.length > 0)
                {
                    _href += '&skill=' + encodeURIComponent(skills);
                }  
                console.log(_href);
				loadContent(_href, function (data) {
					history.pushState({ myTag: true }, null, _href);
    ...
    // jQuery(document).on("keyup", "#search", function (evt){
    //         if(Modernizr.history)
    //         {
    //         var search = $(this).val();
    //         console.log(search);
    //         var _href = "pro_display.php?val=<?php echo $val; ?>";
    //         if(search.length > 0)
    //              _href += '&search=' + encodeURIComponent(search);
    //              console.log(_href);
	// 			loadContent(_href, function (data) {
